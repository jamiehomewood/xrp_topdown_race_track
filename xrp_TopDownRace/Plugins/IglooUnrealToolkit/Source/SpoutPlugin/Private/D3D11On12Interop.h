...
public:
  ...
  DXGI_FORMAT getTextureFormat(const FTextureRHIRef SrcTexture, unsigned int& width, unsigned int& height)
  {
  ...
  void copyToDx11Texture(const FTextureRHIRef Src, ID3D11Texture2D* targetTex)
  {
    FString RHIName = dynamicRHI->GetName();
    if (RHIName == TEXT("D3D12"))
    {
...
private:
  ...
  bool wrapDX12SourceResource(const FTextureRHIRef& Src, ID3D11Resource** Wrapped11Resource)
  {
    HRESULT hr = S_OK;
